...
### LB の `0.1` 差分をどう読むか（概算）

- スコアは `S = sqrt(BLEU * chrF++)` なので、**BLEU と chrF++ が同程度に伸びる**状況では `0.1` の LB 上昇は、だいたい **BLEU +0.1 かつ chrF++ +0.1** に相当する。
- 片方だけが伸びる極端なケースでは、他方が不変なら **片側で約 `+0.2`** ないと LB は `+0.1` にならない。
- Hidden test は **約 4000 文**と案内されているが、Public LB がその全量とは限らない。したがって「何文正解が増えたか」は厳密には言えない。
- `Sentences_Oare_FirstWord_LinNum.csv` の英訳付き行（9771文）を見ると、**1文あたり平均 13.1 語 / 73.6 文字**。これを hidden test に当てると、全量 4000 文で **約 5.9 万語 / 31.3 万文字**、Public 半分想定なら **約 2.9 万語 / 15.7 万文字**。
- スコア 30 前後では、LB `+0.1` は幾何平均としては **約 `+0.33%` の相対改善**。unigram だけで粗く換算すると、Public 半分想定で **約 100 語前後**、全量想定で **約 200 語前後**の追加一致に相当するオーダー感。
- ただし BLEU は 1-4gram、chrF++ は文字 n-gram を見るため、実際には **平均的な1文を丸ごとかなり正しく直す改善なら数文〜十数文規模**、**固有名詞・数字・記号の小修正なら数十文規模**で `+0.1` に届くことがある。
- 特にこのコンペでは **固有名詞、数値、ハイフン、アポストロフ、下付き数字、ギャップ表現**のような“文字一致”が chrF++ を押し上げやすい一方、語順や助動詞、前置詞の改善は BLEU への寄与が大きい。
